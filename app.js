(() => {
  ...
  const plans = [
    ...
    { key:'B', title:'B · Pernas', focus:'Quadríceps, glúteos e cadeia posterior. Necessário para corpo inteiro.', exercises:[
      ...
      { variants:{easy:'Reverse lunge',normal:'Reverse lunge lenta',hard:'Hamstring walkout'}, instructions:{easy:'Passo atrás, desce controlado e regressa.',normal:'Executa lento, alternando lados com controlo.',hard:'Parte da ponte de glúteos e caminha com os calcanhares para a frente e para trás.'}, desc:'Fecho com mais cadeia posterior ou unilateral.', focus:'Quadríceps, glúteos e posteriores.', cues:'Respiração controlada e anca estável.' }
    ]},
  ...
  function updateTechniqueCard(ex){
    const name=currentVariantName(ex, state.exerciseIndex, currentPlan().key);
    const g=exerciseGuide(name);
    el.techTitle.textContent=`Técnica · ${name}`;
    el.techDesc.textContent=g.intro;
    el.techSetup.textContent=g.setup;
  ...
  function renderGuideVisual(){
    const plan=currentPlan(); const idx=Math.min(state.exerciseIndex, plan.exercises.length-1); const ex=plan.exercises[idx]; const name=currentVariantName(ex, idx, plan.key);
  ...
  function renderExerciseList(){
    ...
    el.exerciseList.innerHTML = plan.exercises.map((ex,idx)=>{
      ...
      return `<div class="plan-item ${active ? 'active' : ''}"><div class="left"><div class="name">${name}</div><div class="sub">${g.intro}</div><div class="hint">${g.cues}</div></div><div class="right">${level}</div></div>`;
    }).join('');
  }
